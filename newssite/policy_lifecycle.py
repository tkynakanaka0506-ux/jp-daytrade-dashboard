...
将来の拡張(バックログ、ユーザー提案2026-09-13、今すぐ実装はしない):
本番運用する場合、単に「同じpolicy_event_idかどうか」だけでなく、
NEW(新規発表)/UPDATE(続報)/MATURED(制度成立)/CLOSED(施策実施済み、
またはLIFECYCLE_WINDOW_DAYS経過で追跡終了)のようなライフサイクル状態
そのものを表示・ログ対象にすると、①(織り込み済み検知)②(政策→業績距離)
③(競合比較)と連携しやすくなる(「今どの段階の政策か」を一目で示せる)。
実装するならresolve_policy_event_id()の戻り値にstateを追加し、
active[key]エントリにも状態遷移履歴を持たせる形になる見込み。
"""
import json
