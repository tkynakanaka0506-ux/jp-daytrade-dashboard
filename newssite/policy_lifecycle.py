...
ライフサイクル状態(ユーザー提案2026-09-13、実装済み):
「同じpolicy_event_idかどうか」に加えて、NEW(新規発表)/UPDATE(続報)/
MATURED(制度成立)/CLOSED(施策実施済み、またはLIFECYCLE_WINDOW_DAYS経過で
追跡終了)のライフサイクル状態をresolve_policy_event_id()の戻り値と
active[key]エントリに持たせる。①(織り込み済み検知)②(政策→業績距離)
③(競合比較)と連携しやすくなる(「今どの段階の政策か」を一目で示せる)。

状態の判定は impact.policy_to_earnings_stage() が使う政策成熟度の閾値と
揃えてある(<75=検討〜法案提出はNEW/UPDATE、75以上=法案成立・予算成立を
「制度成立」とみなしMATURED、100=施策実施済みをCLOSED)。CLOSEDはこの他に、
新しい続報が来ないままLIFECYCLE_WINDOW_DAYS経過した場合にもsweep_expired()
で遷移する(resolve_policy_event_id()は該当ニュースが来た時にしか呼ばれない
ため、無音のまま追跡終了するケースは毎回のパイプライン実行時に一括で
検知する必要がある。analyze.py参照)。
"""
import json
...
# impact.policy_to_earnings_stage()の閾値(<75=検討〜法案提出、100=施策実施済み)
# と揃えてある。
MATURED_THRESHOLD = 75
CLOSED_THRESHOLD = 100


def _derive_state(max_maturity, is_new):
    if max_maturity is not None and max_maturity >= CLOSED_THRESHOLD:
        return STATE_CLOSED
    if max_maturity is not None and max_maturity >= MATURED_THRESHOLD:
        return STATE_MATURED
    return STATE_NEW if is_new else STATE_UPDATE
...
def resolve_policy_event_id(registry, theme_id, matched_keyword, maturity_score, title, day):
    ...
    使い回すこと)。戻り値は (policy_event_id, is_update, first_seen, state)。
    """
    ...
    if entry:
        ...
        if gap_days <= LIFECYCLE_WINDOW_DAYS and not regressed:
            ...
            entry["state"] = _derive_state(entry["max_maturity"], is_new=False)
            return entry["policy_event_id"], True, entry["first_seen"], entry["state"]
    ...
    new_entry = {
        ...
        "update_count": 1,
        "titles": [title],
        "state": _derive_state(maturity_score, is_new=True),
    }
    active[key] = new_entry
    return new_entry["policy_event_id"], False, new_entry["first_seen"], new_entry["state"]


def sweep_expired(registry, today):
    """新しい続報が来ないままLIFECYCLE_WINDOW_DAYS経過したactiveエントリをCLOSEDにする。

    resolve_policy_event_id()は該当テーマのニュースが実際に来た時にしか
    呼ばれないため、無音のまま追跡終了するケースはここで検知する必要がある。
    毎回のパイプライン実行時に(該当ニュースの有無に関わらず)呼ぶ想定。
    """
    active = registry.setdefault("active", {})
    for entry in active.values():
        if entry.get("state") == STATE_CLOSED:
            continue
        if _days_between(entry["last_seen"], today) > LIFECYCLE_WINDOW_DAYS:
